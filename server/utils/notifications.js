...
const sendEmail = async (type, to, subject, data, retryCount = 0) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"Glowing Art" <${process.env.EMAIL_USER}>`,
      to,
      subject,
      html: htmlContent,
      headers: {
        "X-Mailer": "Glowing Art Order System",
        "X-Priority": "3",
        ...
        "Message-ID": `<${Date.now()}-${Math.random().toString(36).substr(2, 9)}@glowing-art.com>`,
      },
      text: `
...
Thank you for choosing Glowing Art!
    ...
    if (retryCount < 3 && (error.code === "ECONNECTION" || error.code === "ETIMEDOUT" || error.code === "ESOCKET")) {
      console.log(`🔄 Retrying email send (attempt ${retryCount + 1}/3) in ${(retryCount + 1) * 2} seconds...`)
