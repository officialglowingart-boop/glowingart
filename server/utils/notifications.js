...
//       maxConnections: 5,
//       maxMessages: 100,
//       rateLimit: 10, // Reduced rate limit for better reliability
//       rateDelta: 1000,
//       requireTLS: true,
...
//     const messageId = `<${Date.now()}-${Math.random().toString(36).substr(2, 9)}@glowing-gallery.com>`
...
//         "Authentication-Results": `glowing-gallery.com; spf=pass; dkim=pass; dmarc=pass`,
//       },
//       text: `
  ...
  try {
    ...
    const emailPromise = sendEmail(
      ...
      .then(() => {
        ...
        console.log(`✅ Email accepted by server for: ${order.customerInfo.email}`)
        console.log(`Order confirmation sent for ${order.orderNumber}`)
        console.log(`📧 Order confirmation sent successfully for: ${order.orderNumber}`)
      })
      .catch((error) => {
        console.error(`❌ Email failed for ${order.orderNumber}:`, error.message)
      })

    const whatsappPromise = sendWhatsApp("orderConfirmation", order.customerInfo.phone, whatsappData)
      .then(() => {
    ...
    Promise.all([emailPromise, whatsappPromise]).catch(() => {
      // Silently handle any remaining errors to prevent unhandled promise rejections
    })

    // Return immediately without waiting for email completion
    return { success: true, message: "Notifications initiated" }
  } catch (error) {
    console.error("Order confirmation error:", error)
...
Thank you for choosing Glowing Gallery!
    ...
    const retryableErrors = ["ECONNECTION", "ETIMEDOUT", "ESOCKET", "ENOTFOUND", "ECONNRESET"]
    if (retryCount < 1 && (retryableErrors.includes(error.code) || error.message.includes("timeout"))) {
      const delay = 1000 // Fixed 1 second delay instead of exponential backoff
      console.log(`🔄 Retrying email send (attempt ${retryCount + 1}/1) in ${delay / 1000} seconds...`)
      await new Promise((resolve) => setTimeout(resolve, delay))
      return sendEmail(type, to, subject, data, retryCount + 1)
    }
