...
const sendEmail = async (type, to, subject, data, isMobile = false) => {
  if (!transporter) {
  ...
  try {
    ...
    console.log(`📧 [${isMobile ? "MOBILE" : "DESKTOP"}] Sending email to ${to}: ${subject}`)

    let lastError
    const maxAttempts = isMobile ? 2 : 3 // Fewer attempts for mobile
    const baseDelay = isMobile ? 500 : 1000 // Faster retries for mobile (0.5s vs 1s base)

    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        await transporter.sendMail(mailOptions)
        console.log(`✅ [${isMobile ? "MOBILE" : "DESKTOP"}] Email sent successfully to ${to} (attempt ${attempt})`)
        return // Success, exit retry loop
      } catch (attemptError) {
        lastError = attemptError
        console.warn(
          `⚠️ [${isMobile ? "MOBILE" : "DESKTOP"}] Email attempt ${attempt} failed for ${to}:`,
          attemptError.message,
        )

        if (attempt < maxAttempts) {
          const delay = Math.pow(2, attempt) * baseDelay // Mobile: 1s, 2s vs Desktop: 2s, 4s
          console.log(`⏳ [${isMobile ? "MOBILE" : "DESKTOP"}] Retrying email in ${delay}ms...`)
    ...
    console.error(`❌ [${isMobile ? "MOBILE" : "DESKTOP"}] Email sending failed after all retries:`, error.message)
    ...
    if (error.code === "EAUTH") {
      ...
      console.error("   Connection timed out - network or server issue")
...
const sendOrderConfirmation = async (order, isMobile = false) => {
  try {
    ...
    console.log(`📧 [${isMobile ? "MOBILE" : "DESKTOP"}] Order confirmation sent for ${order.orderNumber}`)
  } catch (error) {
    console.error(`❌ [${isMobile ? "MOBILE" : "DESKTOP"}] Order confirmation error:`, error)
    throw error
  }
}
